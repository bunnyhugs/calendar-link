...
ready(function() {
	const calendarLinkElements = document.querySelectorAll("[data-calendar-title]");
			calendarLinkElements.forEach(link => {
				...
				let includeHeader  = link.getAttribute("data-calendar-includeHeader");
				...
				let eventLinks = "<ul>";
				eventLinks += "<li style=\"display: block; margin-top: 25px; font-weight: 600;\">";
				if (includeHeader) {
					eventLinks += "<span style=\"font-weight: 600; display: inline-block; line-height: 1; padding: 8px 20px; margin-right: 10px; fill: #fff; color: #fff; font-size: 13px; text-transform: uppercase; letter-spacing: 1.4px; background-color: #cc2038; border-radius: 100px 100px 100px 100px; box-shadow: 0px 0px 27px 0px rgb(0 0 0 / 12%);\">Add event to your calendar</span><br /><br />";
				}
				eventLinks +=  title + "</li>";
